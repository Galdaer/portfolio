...
class BillingEngineService:
    ...
    async def submit_claim(
        ...
        try:
            ...
            logger.exception(f"Claim submission failed: {e}")
            return {
    ...
    async def validate_codes(
        ...
        try:
            ...
            logger.exception(f"Code validation failed: {e}")
            return {
    ...
    async def get_claim_status(
        ...
        try:
            ...
            logger.exception(f"Claim status retrieval failed: {e}")
            return {
    ...
    async def _get_claim_info(self, claim_id: str) -> ClaimResult | None:
...
async def submit_claim(request: ClaimRequest):
    ...
    try:
        return await billing_service.submit_claim(request)
    except Exception as e:
        logger.exception(f"Claim submission endpoint error: {e}")
...
async def validate_codes(request: CodeValidationRequest):
    ...
    try:
        return await billing_service.validate_codes(request)
    except Exception as e:
        logger.exception(f"Code validation endpoint error: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_claim_status(claim_id: str):
    """Get claim status"""
    try:
        return await billing_service.get_claim_status(claim_id)
    except Exception as e:
        logger.exception(f"Claim status endpoint error: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def process_payment(request: PaymentRequest):
    ...
    try:
        ...
        logger.exception(f"Payment endpoint error: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def get_billing_analytics(
    ...
    provider_id: str | None = Query(None, description="Provider ID filter"),
):
    ...
    try:
        ...
        return await billing_service.generate_billing_analytics(
            start_dt, end_dt, provider_id,
        )
    except Exception as e:
        logger.exception(f"Analytics endpoint error: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
