...
if [[ ! -f "$MCP_CONFIG_PATH" ]]; then
    echo "❌ MCP configuration not found: $MCP_CONFIG_PATH"
    exit 1
fi

echo "✅ MCP configuration found: $MCP_CONFIG_PATH"

echo "🔍 Skipping active wait for Healthcare MCP container (will attempt lazy connect)"
echo "🧪 (Optional) MCP server version check..."
# Only attempt if docker socket present (avoid noisy errors in restricted envs)
if [[ -S /var/run/docker.sock ]]; then
    if docker exec -i "${HEALTHCARE_MCP_CONTAINER:-healthcare-mcp-stdio}" node /app/build/index.js --version 2>/dev/null; then
            echo "✅ MCP server connection test passed"
    else
            echo "⚠️ MCP server version check not available now; continuing (lazy connect)"
    fi
else
    echo "ℹ️ Skipping version check (no docker socket)"
fi

# Start the pipeline server
echo "🚀 Starting MCP Pipeline server on ${PIPELINES_HOST}:${PIPELINES_PORT}"

cd /app/pipelines

# Create a simple FastAPI server to host our pipeline
cat > pipeline_server.py << 'EOF'
"""
...
# Add route alias for Open WebUI compatibility
