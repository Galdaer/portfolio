...
class DrugConsolidationEngine:
    ...
    def normalize_generic_name(self, generic_name: Optional[str]) -> Optional[str]:
    ...
    def get_generic_drug_groups(self, limit: Optional[int] = None) -> List[Tuple[str, int]]:
    ...
    def consolidate_clinical_text_field(self, records: List[DrugInformation], field_name: str) -> Optional[str]:
    ...
    def consolidate_therapeutic_class(self, records: List[DrugInformation]) -> Optional[str]:
    ...
    def consolidate_drug_group(self, normalized_generic: str, records: List[DrugInformation]) -> DrugInformation:
        ...
        consolidated_data = {
            ...
            'approval_dates': sorted(list(approval_dates)),
            'orange_book_codes': sorted(list(orange_book_codes)),
            'application_numbers': sorted(list(application_numbers)),
            
            # Metadata
            'total_formulations': len(formulations_list),
            'data_sources': sorted(list(all_data_sources))
        }
    ...
    def consolidate_all_drugs(self, batch_size: int = 1000, start_offset: int = 0) -> Dict[str, Any]:
        ...
        for i, (normalized_generic, record_count) in enumerate(drug_groups[start_offset:], start_offset):
            try:
                ...
                logger.error(f"Error consolidating {normalized_generic}: {e}")
                errors += 1
                continue
