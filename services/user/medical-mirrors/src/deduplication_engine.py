...
from database import ClinicalTrial, get_connection_pool_status, get_thread_safe_session
...
class CrossBatchDeduplicator:
    ...
    async def process_clinical_trials_batch(self, trials: list[dict[str, Any]],
                                          existing_keys: set[str] | None = None) -> dict[str, Any]:
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    self.logger.exception(f"Failed to bulk insert clinical trials after {max_retries} attempts due to deadlocks")
                    raise
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    self.logger.exception(f"Failed to bulk update clinical trials after {max_retries} attempts due to deadlocks")
                    raise
        ...
        for attempt in range(max_retries):
            try:
                ...
                if attempt < max_retries - 1:
                    ...
                    self.logger.exception(f"Failed to update search vectors after {max_retries} attempts due to deadlocks")
...
class DeduplicationProgressTracker:
    ...
    def log_progress(self) -> None:
        ...
        self.logger.info("📈 Processing Progress Report:")
...
class SmartBatchProcessor:
    ...
    async def process_clinical_trials_files(self, json_files: list[str],
        ...
        self.logger.info("🎉 Smart batch processing completed!")
