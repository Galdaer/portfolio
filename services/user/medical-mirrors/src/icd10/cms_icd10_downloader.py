...
class CMSICD10Downloader:
    ...
    def __init__(self, state_manager: ICD10DownloadStateManager | None = None,
    ...
    def _parse_icd10_zip(self, zip_content: bytes, source: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.exception(f"Invalid ZIP file for ICD-10 {source}")
        except Exception as e:
            logger.exception(f"Error parsing ICD-10 ZIP file for {source}: {e}")
    ...
    def _parse_icd10_xml(self, xml_content: str, filename: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.exception(f"XML parsing error in {filename}: {e}")
        except Exception as e:
            logger.exception(f"Error parsing ICD-10 XML from {filename}: {e}")
    ...
    def _parse_icd10_text(self, text_content: str, filename: str) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.exception(f"Error parsing ICD-10 text from {filename}: {e}")
    ...
    async def _save_codes_to_file(self, codes: list[dict[str, Any]], output_file: Path):
        ...
        try:
            ...
            logger.exception(f"Error saving ICD-10 codes to {output_file}: {e}")
