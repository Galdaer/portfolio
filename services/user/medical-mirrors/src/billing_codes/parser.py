...
class BillingCodesParser:
    ...
    def parse_and_validate(self, raw_codes: List[Dict]) -> List[Dict]:
        ...
        for raw_code in raw_codes:
            try:
                ...
                logger.error(f"Error parsing billing code {raw_code.get('code', 'unknown')}: {e}")
                self.validation_errors += 1
                continue
    ...
    def _parse_single_code(self, raw_code: Dict) -> Optional[Dict]:
        ...
        try:
            ...
            logger.error(f"Error parsing single billing code: {e}")
            return None
    ...
    def _check_gender_specificity(self, description: str) -> Optional[str]:
        """Check if code is gender-specific"""
        desc_lower = description.lower()
        
        if any(term in desc_lower for term in ["female", "woman", "maternal", "pregnancy", "obstetric"]):
            return "female"
        elif any(term in desc_lower for term in ["male", "man", "prostate", "testicular"]):
            return "male"
        
        return None
    
    def _check_age_specificity(self, description: str) -> Optional[str]:
        """Check if code is age-specific"""
        desc_lower = description.lower()
        
        if any(term in desc_lower for term in ["pediatric", "child", "infant", "newborn"]):
            return "pediatric"
        elif any(term in desc_lower for term in ["adult", "geriatric", "elderly"]):
            return "adult"
    ...
    def _validate_code(self, parsed_code: Dict) -> bool:
        ...
        try:
            ...
            if len(description) < 3 or len(description) > 1000:
                return False
