...
class BillingCodesAPI:
    ...
    async def search_codes(
        ...
        try:
            ...
            logger.exception(f"Error searching billing codes: {e}")
    ...
    async def get_categories(self, code_type: str | None = None) -> dict:
...
async def search_billing_codes(
    ...
    code_type: str | None = Query(None, description="Filter by code type (CPT, HCPCS)"),
...
async def get_billing_categories(
    code_type: str | None = Query(None, description="Filter by code type"),
):
    """Get all billing code categories"""
