...
class ExerciseRateLimitManager:
    ...
    def save_rate_limit_status(self):
        ...
        try:
            ...
            logger.exception(f"Error saving rate limit status: {e}")
...
class ExerciseCacheManager:
    ...
    async def get_cached_exercise(self, exercise_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if result:
                ...
                exercise_data = dict(zip(columns, result, strict=False))
                logger.debug(f"Found cached exercise: {exercise_id}")
                return exercise_data

            return None
        except Exception as e:
            logger.exception(f"Error querying cached exercise {exercise_id}: {e}")
            return None
        finally:
            db.close()
    ...
    async def cache_exercise(self, exercise_data: dict[str, Any]) -> bool:
        ...
        try:
            ...
            logger.exception(f"Error caching exercise: {e}")
            db.rollback()
            return False
        finally:
            db.close()

    async def fetch_exercise_from_api(self, exercise_id: str) -> dict[str, Any] | None:
        ...
        try:
            ...
            if e.status == 429:
                self.rate_limiter.record_rate_limit()
            logger.exception(f"HTTP error fetching exercise {exercise_id}: {e}")
            return None
        except Exception as e:
            logger.exception(f"Error fetching exercise {exercise_id}: {e}")
            return None
    ...
    async def search_cached_exercises(self, query: str, limit: int = 20) -> list[dict[str, Any]]:
        ...
        try:
            ...
            logger.exception(f"Error searching cached exercises: {e}")
            return []
        finally:
            db.close()
