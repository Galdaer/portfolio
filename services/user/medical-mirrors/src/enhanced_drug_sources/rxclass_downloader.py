...
class SmartRxClassDownloader:
    ...
    def _save_state(self):
        ...
        try:
            ...
            logger.exception(f"Failed to save RxClass state: {e}")
    ...
    async def get_drug_classes(
        ...
        try:
            ...
            logger.exception(f"HTTP error getting RxClass data for {drug_name}: {e}")
            self.state.failed_downloads += 1
            return None

        except Exception as e:
            logger.exception(f"Error getting RxClass data for {drug_name}: {e}")
            self.state.failed_downloads += 1
            return None
    ...
    async def get_class_members(
        ...
        try:
            ...
            logger.exception(f"HTTP error getting RxClass members for {class_id}: {e}")
            self.state.failed_downloads += 1
            return None

        except Exception as e:
            logger.exception(f"Error getting RxClass members for {class_id}: {e}")
            self.state.failed_downloads += 1
            return None
