...
class SmartDDInterDownloader:
    ...
    def _save_state(self):
        ...
        try:
            ...
            logger.exception(f"Failed to save DDInter state: {e}")
    ...
    async def search_drug_interactions(
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                logger.exception(f"HTTP error searching DDInter for {drug_name}: {e}")
                self.state.failed_downloads += 1
            return None

        except Exception as e:
            logger.exception(f"Error searching DDInter for {drug_name}: {e}")
            self.state.failed_downloads += 1
            return None
