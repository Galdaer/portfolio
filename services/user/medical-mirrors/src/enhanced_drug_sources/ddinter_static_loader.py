...
class DDInterStaticLoader:
    ...
    def detect_file_format(self, file_path: Path) -> str | None:
    ...
    def load_from_sqlite(self, file_path: Path) -> list[dict]:
        ...
        try:
            ...
            logger.exception(f"Error loading SQLite file {file_path}: {e}")
    ...
    def load_from_json(self, file_path: Path) -> list[dict]:
        ...
        try:
            ...
            if isinstance(data, list):
                ...
                msg = f"Unexpected JSON structure: {type(data)}"
                raise Exception(msg)
    ...
    def load_from_zip(self, file_path: Path) -> list[dict]:
        ...
        try:
            ...
            logger.exception(f"Error loading ZIP file {file_path}: {e}")
