...
class SmartClinicalTrialsDownloader:
    ...
    def _save_state(self):
        ...
        try:
            ...
            logger.exception(f"Failed to save ClinicalTrials state: {e}")
    ...
    async def search_drug_studies(
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                logger.exception(f"HTTP error searching for {drug_name}: {e}")
                self.state.failed_downloads += 1
            return None

        except Exception as e:
            logger.exception(f"Error searching ClinicalTrials for {drug_name}: {e}")
            self.state.failed_downloads += 1
            return None
