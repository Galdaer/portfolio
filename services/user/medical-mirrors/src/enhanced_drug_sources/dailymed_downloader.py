...
class SmartDailyMedDownloader:
    ...
    def _save_state(self):
        ...
        try:
            ...
            logger.exception(f"Failed to save DailyMed state: {e}")
    ...
    async def download_drug_labeling_by_setid(self, setid: str) -> str | None:
        ...
        try:
            ...
            if e.response.status_code == 404:
                ...
                logger.exception(f"HTTP error downloading setId {setid}: {e}")
                self.state.failed_downloads += 1
            return None

        except Exception as e:
            logger.exception(f"Error downloading DailyMed setId {setid}: {e}")
            self.state.failed_downloads += 1
            return None
    ...
    async def search_drugs_for_download(self, drug_names: list[str], max_per_drug: int = 10) -> list[str]:
        ...
        for drug_name in drug_names:
            try:
                ...
                logger.exception(f"Error searching for drug {drug_name}: {e}")
                continue
