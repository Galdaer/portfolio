...
def safe_parse(
    ...
    try:
        ...
        parse_logger.error(f"Unexpected error parsing record {record_id}: {e}")
...
class ErrorCollector:
    ...
    def record_error(self, error: Exception, record_id: Optional[str] = None, context: Optional[str] = None):
...
def handle_database_errors(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            raise DatabaseError(f"Transient database error: {e}")
            ...
            logger.error(f"Database data error in {func.__name__}: {e}")
            raise DatabaseError(f"Data format error: {e}")
        except Exception as e:
...
def log_function_performance(func: Callable) -> Callable:
    ...
    def wrapper(*args, **kwargs):
        ...
        try:
            ...
            execution_time = time.time() - start_time
            func_logger.error(f"Function {func.__name__} failed after {execution_time:.2f}s: {e}")
            raise
