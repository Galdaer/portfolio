...
class HealthcareComplianceConfig(BaseModel):
    ...
    max_failed_auth_attempts: int = Field(
        default=3,
        description="Max failed authentication attempts",
    )
...
class CacheConfig(BaseModel):
    ...
    medical_literature_ttl_hours: int = Field(
        default=24,
        description="Medical literature cache TTL",
    )
...
class DatabaseConfig(BaseModel):
    ...
    postgres_url: str = Field(
        default="postgresql://172.20.0.13:5432/intelluxe",
...
class MonitoringConfig(BaseModel):
    ...
    alert_threshold_error_rate: float = Field(
        default=0.05,
        description="Error rate alert threshold (5%)",
    )
    alert_threshold_response_time_ms: int = Field(
        default=5000,
        description="Response time alert threshold",
    )
            ...
            if isinstance(database_config, dict):
                database_config["postgres_url"] = os.getenv(
                    ...
                    config_data.get("database", {}).get(
                        "postgres_url",
                        "postgresql://172.20.0.13:5432/intelluxe",
                    )
