...
class HealthcareMCPClient:
    ...
    def __init__(self, server_command: str | None = None):
        ...
        if os.path.exists(self.mcp_server_path):
            ...
            logger.info(f"MCP environment configured with API keys and service URLs: {list(self.env.keys())}")
    ...
    async def call_tool(self, tool_name: str, arguments: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            logger.debug(f"MCP initialized successfully: {init_response.get('result', {}).get('serverInfo', {})}")
            ...
            logger.error(f"CRITICAL DEBUG - Tool message being sent: {json.dumps(tool_msg, indent=2)}")
            ...
            logger.info(f"DEBUG: Raw tool response for {tool_name}: {json.dumps(tool_response, indent=2)[:500]}...")
            ...
            logger.info(f"DEBUG: Extracted result for {tool_name}: {json.dumps(result, indent=2)[:500]}...")
