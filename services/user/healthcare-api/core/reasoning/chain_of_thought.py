...
class ChainOfThoughtProcessor:
    ...
    async def process_reasoning_chain(
        ...
        if not template:
            msg = f"No template available for reasoning type: {reasoning_type}"
    ...
    async def _process_reasoning_step(
        ...
        try:
            ...
            logger.exception(f"Error processing reasoning step {step_id}: {e}")
            # Return error step
            return ReasoningStep(
                step_id=step_id,
                step_type=step_template["step_type"],
    ...
    async def _synthesize_reasoning_chain(
        ...
        try:
            ...
            weighted_confidence = total_weight / len(steps) if total_weight > 0 else 0.0
            ...
            logger.exception(f"Error synthesizing reasoning chain: {e}")
            return {
