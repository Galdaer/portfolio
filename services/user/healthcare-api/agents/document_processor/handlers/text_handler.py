...
class TextDocumentHandler(BaseDocumentHandler):
    ...
    async def extract_content(self, file_path: Union[str, Path]) -> str:
        ...
        try:
            ...
            if content is None:
                raise DocumentProcessingError(
                    f"Could not decode text file with supported encodings: {self.supported_encodings}",
                    str(file_path)
                )
    ...
    async def extract_metadata(self, file_path: Union[str, Path]) -> DocumentMetadata:
        ...
        try:
            ...
            raise DocumentProcessingError(
                f"Failed to extract metadata from text file: {e}",
                str(file_path),
                e
            )
    ...
    def _detect_content_type(self, content: str) -> str:
        ...
        if ',' in content and content.count(',') > content.count('\n'):
            ...
            return 'markdown'
        elif any(indicator in content_lower for indicator in medical_indicators):
            return 'medical_text'
        elif 'log' in content_lower or any(level in content_lower for level in ['error', 'warn', 'info', 'debug']):
            return 'log_file'
        else:
            return 'plain_text'
    ...
    async def analyze_text_structure(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        """
