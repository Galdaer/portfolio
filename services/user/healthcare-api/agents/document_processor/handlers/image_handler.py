...
class ImageDocumentHandler(BaseDocumentHandler):
    ...
    async def extract_content(self, file_path: Union[str, Path]) -> str:
        ...
        try:
            ...
            raise DocumentProcessingError(
                f"Failed to extract content from image: {e}",
                str(file_path),
                e
            )
    ...
    async def _clean_ocr_text(self, ocr_text: str) -> str:
        ...
        replacements = [
            ...
            ('0', 'O'),  # When context suggests letter over number
            # Add more OCR correction rules as needed
        ]
    ...
    async def extract_ocr_data(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise DocumentProcessingError(
                f"Failed to extract OCR data from image: {e}",
                str(file_path),
                e
            )
    
    async def detect_document_regions(self, file_path: Union[str, Path]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            raise DocumentProcessingError(
                f"Failed to detect document regions in image: {e}",
                str(file_path),
                e
            )