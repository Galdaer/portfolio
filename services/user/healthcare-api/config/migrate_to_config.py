...
class AgentConfigMigrator:
    ...
    def _migrate_agent_file(self, file_path: Path) -> bool:
        ...
        try:
            ...
            logger.exception(f"Failed to migrate {file_path}: {str(e)}")
            return False
...
from config.config_loader import get_healthcare_config"""
        ...
        return re.sub(compile_result_pattern, new_compile_method, content, flags=re.DOTALL)
    ...
    def validate_migration(self) -> bool:
        ...
        try:
            ...
            logger.exception(f"❌ Configuration migration validation failed: {str(e)}")
            return False
