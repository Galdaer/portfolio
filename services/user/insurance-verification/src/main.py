...
class InsuranceVerificationService:
    ...
    async def verify_eligibility(
        ...
        try:
            ...
            logger.exception(f"Verification failed for provider {provider_name}: {e}")
            return {
    ...
    async def request_prior_authorization(
        ...
        try:
            ...
            logger.exception(f"Prior auth failed for provider {provider_name}: {e}")
            return {
    ...
    async def inquire_benefits(
        ...
        try:
            ...
            logger.exception(f"Benefits inquiry failed for provider {provider_name}: {e}")
            return {
    ...
    async def _detect_provider(self, member_id: str, provider_id: str) -> str:
        ...
        if "ant" in provider_id_lower or member_id.startswith(("A", "B", "C")):
            return "anthem"
        if "uhc" in provider_id_lower or "united" in provider_id_lower:
            return "uhc"
        if "cigna" in provider_id_lower:
            return "cigna"
        if "aetna" in provider_id_lower:
            return "aetna"
...
async def verify_insurance(request: InsuranceVerificationRequest):
    ...
    try:
        return await verification_service.verify_eligibility(request)
    except Exception as e:
...
async def request_prior_authorization(request: PriorAuthRequest):
    ...
    try:
        return await verification_service.request_prior_authorization(request)
    except Exception as e:
        logger.exception(f"Prior auth endpoint error: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
...
async def inquire_benefits(request: BenefitsInquiryRequest):
    """Inquire about member benefits"""
    try:
        return await verification_service.inquire_benefits(request)
    except Exception as e:
        logger.exception(f"Benefits endpoint error: {e}")
        raise HTTPException(status_code=500, detail="Internal server error")
