...
class BaseInsuranceProvider(ABC):
    ...
    async def _make_request(
        ...
        try:
            ...
            logger.exception(f"Timeout calling {self.provider_name} API at {url}")
            return ProviderResponse(
                provider_name=self.provider_name,
            ...
            logger.exception(f"Request error calling {self.provider_name} API: {e}")
            return ProviderResponse(
                provider_name=self.provider_name,
            ...
            logger.exception(f"Unexpected error calling {self.provider_name} API: {e}")
            return ProviderResponse(
                provider_name=self.provider_name,
