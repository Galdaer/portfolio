...
'''
        ...
        for i, line in enumerate(lines):
            if line.startswith(("class Action:", "class Filter:", "class Pipe:")):
                class_line = i
                break
    ...
    def convert_to_json(self, py_file_path: str, output_path: str | None = None) -> dict[str, Any]:
        """Convert Python function file to Open WebUI JSON format."""
        py_path = Path(py_file_path)

        if not py_path.exists():
            msg = f"Python file not found: {py_file_path}"
