...
class PHIMonitor:
    ...
    def scan_for_phi(
        ...
        detected_phi_types = []
        detection_details = {'matches': {}, 'context': context}
        ...
        for phi_type, patterns in self._compiled_patterns.items():
            ...
            if matches:
                ...
                detection_details['matches'][phi_type.value] = len(matches)
    ...
    def _prepare_scan_text(self, data: Union[str, Dict[str, Any], List[Any]]) -> str:
    ...
    def _scan_field_names(self, data: Union[str, Dict[str, Any], List[Any]]) -> List[str]:
    ...
    def _log_phi_detection(self, result: PHIDetectionResult, context: Optional[str]) -> None:
        ...
        if phi_result.phi_detected:
            if phi_result.risk_level == PHIRiskLevel.CRITICAL:
                ...
                validation_result['errors'].append(
                    "Critical PHI detected in healthcare context"
                )
            elif phi_result.risk_level == PHIRiskLevel.HIGH:
                validation_result['warnings'].append(
                    "High-risk PHI detected in healthcare context"
                )
...
def scan_for_phi(data: Any, context: Optional[str] = None) -> bool:
    """
