...
class HealthcareCacheManager:
    ...
    async def set(
        ...
        try:
            ...
            if phi_detected and security_level not in [CacheSecurityLevel.PHI_PROTECTED, CacheSecurityLevel.NO_CACHE]:
                logger.error(
                    f"PHI detected in data for non-PHI cache level",
                    extra={
    ...
    async def _detect_phi_in_data(self, data: Any) -> bool:
        ...
        phi_patterns = [
            'ssn', 'social security', 'patient_id', 'medical_record',
...
_cache_manager: Optional[HealthcareCacheManager] = None
