...
class ChatLogManager:
    ...
    async def log_chat_message(
        ...
        if phi_detected:
            ...
            logger.warning(
                f"PHI detected in chat message",
                extra={
        ...
        for message in session.messages[-max_messages:]:
            if message.phi_detected and not include_phi:
                ...
                sanitized_message = ChatMessage(
                    ...
                    audit_trail=message.audit_trail + [f"PHI_filtered_at_{datetime.utcnow().isoformat()}"]
                )
    ...
    async def end_session(self, session_id: str, user_id: str) -> Dict[str, Any]:
        ...
        logger.info(
            ...
            extra={
                'operation_type': 'chat_session_ended',
    ...
    async def quarantine_message(
        ...
        logger.critical(
            f"Message quarantined due to PHI exposure",
            extra={
    ...
    def _determine_security_level(
        ...
        content: str, 
        phi_detected: bool, 
        healthcare_context: Optional[Dict[str, Any]]
    ) -> ChatLogLevel:
        ...
        healthcare_keywords = [
            ...
            'medical', 'hospital', 'doctor', 'nurse', 'clinic'
        ]
    ...
    async def _handle_session_message(
        ...
        if message.phi_detected:
            ...
            if session.phi_detected_count > 3:
                ...
                logger.warning(
                    f"PHI threshold exceeded in session",
                    extra={
