...
class MockLLMClient:
    ...
    async def generate(self, model: str, prompt: str, **kwargs) -> Dict[str, Any]:
        """Mock LLM generation"""
        return {
            "response": f"Mock medical analysis for: {prompt[:100]}...",
            "done": True
        }
...
async def test_enhanced_medical_query_engine_integration():
    ...
    try:
        ...
        from agents.medical_search_agent.medical_search_agent import MedicalLiteratureSearchAssistant, MedicalSearchResult
    ...
    try:
        ...
        test_queries = [
            ...
            ("differential diagnosis for chest pain", "differential_diagnosis", QueryType.DIFFERENTIAL_DIAGNOSIS),
            ("clinical guidelines for hypertension", "clinical_guidelines", QueryType.CLINICAL_GUIDELINES),
            ("recent research on cancer treatment", "information_request", QueryType.LITERATURE_RESEARCH),
        ]
...
async def test_enhanced_query_engine_direct():
    ...
    try:
        ...
        try:
            ...
            print(f"⚠️  Direct query processing failed (expected in test environment): {direct_error}")
