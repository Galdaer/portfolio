...
class ContainerHealthcareTest:
    ...
    def ensure_container_built(self) -> bool:
        ...
        try:
            ...
            ], cwd="/home/intelluxe", capture_output=True, text=True, timeout=300)
    ...
    def test_mcp_server_in_container(self) -> bool:
        ...
        try:
            cmd = [
                "docker", "run", "--rm", "-i",
                self.image_name,
                "node", "/app/mcp-server/build/stdio_entry.js"
            ]
            input_data = '{"jsonrpc": "2.0", "id": 1, "method": "tools/list", "params": {}}\n'
            result = subprocess.run(cmd, input=input_data,
                                    capture_output=True, text=True, timeout=30)
...
def main():
    ...
    if "--quick" in sys.argv:
        ...
        success = (test.ensure_container_built()
                   and test.test_mcp_server_in_container())
    else:
        # Full test suite
        test = ContainerHealthcareTest()
