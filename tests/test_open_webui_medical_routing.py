...
class OpenWebUIMedicalRoutingTest:
    ...
    def check_log_activity(self, baseline: Dict[str, int], expected_logs: List[str]) -> Dict[str, bool]:
    ...
    async def test_direct_medical_query_api(self) -> bool:
        ...
        medical_query = {
            ...
            "messages": [
                {
                    "role": "user",
                    "content": "What are the symptoms of diabetes mellitus type 2?"
                }
            ],
            "stream": False
        }
                ...
                async with session.post(
                    ...
                    if response.status == 200:
                        ...
                        log_activity = self.check_log_activity(
                            baseline, 
                            ["medical", "search", "agent", "diabetes", "pubmed"]
                        )
