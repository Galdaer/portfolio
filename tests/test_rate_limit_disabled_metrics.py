...
def test_rate_limit_disabled_metrics(monkeypatch, test_client: TestClient):
    ...
    disabled_line = next((line for line in body if line.startswith("healthcare_rate_limit_disabled")), None)
    assert disabled_line == "healthcare_rate_limit_disabled 1"
    # Ensure standard rate limit counters absent when disabled
    assert not any(line.startswith("healthcare_rate_limit_total") for line in body), "rate_limit_total should be suppressed when disabled"
    assert not any(line.startswith("healthcare_rate_limit_breakdown_total") for line in body), "breakdown metrics should be suppressed when disabled"
