...
# Healthcare Agent Coordination Patterns
...
def route_to_agent(request: str) -> str:
    keywords = {
        "search": ["research", "articles", "studies", "pubmed"],
        "intake": ["patient", "admission", "intake", "new"],
        "document": ["process", "analyze", "extract", "document"]
    }
    
    for agent_type, agent_keywords in keywords.items():
        if any(keyword in request.lower() for keyword in agent_keywords):
...
```python
# HTTP Request → Agent Router → Specific Agent → MCP Tools
async def process_healthcare_request(request: str) -> Dict[str, Any]:
    # Route to appropriate agent
    agent_name = route_to_agent(request)
    agent = get_agent_instance(agent_name)
    
    # Process with medical safety
