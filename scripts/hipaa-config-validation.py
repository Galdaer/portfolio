...
class HIPAAConfigValidator:
        ...
        return {
            "administrative_safeguards": {
                ...
                "validation_checks": {
                    ...
                    "workforce_training_program": lambda x: isinstance(x, dict) and "frequency_months" in x,
            ...
            "physical_safeguards": {
                ...
                "validation_checks": {
                    "facility_access_controls": lambda x: isinstance(x, dict) and "access_logging" in x,
                    "workstation_use_restrictions": lambda x: isinstance(x, dict) and "automatic_logoff" in x
                }
            },
            "technical_safeguards": {
                ...
                "validation_checks": {
                    "access_control": lambda x: isinstance(x, dict) and "unique_user_identification" in x,
                    "audit_controls": lambda x: isinstance(x, dict) and "log_retention_days" in x and x["log_retention_days"] >= 2555,
                    "transmission_security": lambda x: isinstance(x, dict) and x.get("tls_version") == "1.3"
                }
            },
            "encryption": {
                "required": True,
                ...
                "validation_checks": {
                    "data_at_rest": lambda x: isinstance(x, dict) and x.get("algorithm") == "AES-256-GCM",
                    "data_in_transit": lambda x: isinstance(x, dict) and x.get("tls_version") == "1.3",
                    "key_management": lambda x: isinstance(x, dict) and "rotation_days" in x and x["rotation_days"] <= 365
                }
            },
            "audit_monitoring": {
                "required": True,
                ...
                "validation_checks": {
                    ...
                    "log_retention_policy": lambda x: isinstance(x, dict) and x.get("retention_days", 0) >= 2555,
                    "real_time_monitoring": lambda x: isinstance(x, dict) and x.get("enabled") is True,
            ...
            "breach_notification": {
                ...
                "validation_checks": {
                    "notification_procedures": lambda x: isinstance(x, dict) and "timeline_hours" in x and x["timeline_hours"] <= 72,
                    "risk_assessment_process": lambda x: isinstance(x, dict) and "automated_assessment" in x
                }
            },
                ...
                for root, dirs, files in os.walk(search_path):
                    ...
                    for file in files:
                        ...
                        if any(keyword in file.lower() for keyword in [
                            "hipaa", "compliance", "security", "healthcare_settings"
                        ]):
        ...
        try:
            with open(config_file, 'r') as f:
                if config_file.endswith(('.yml', '.yaml')):
                    ...
                    issues.append(HIPAAValidationIssue(
    ...
    def _validate_hipaa_sections(self, config_data: Dict[str, Any], config_file: str) -> List[HIPAAValidationIssue]:
    ...
    def _validate_healthcare_system_config(self, config_data: Dict[str, Any], config_file: str) -> List[HIPAAValidationIssue]:
    ...
    def _validate_environment_requirements(self, config_data: Dict[str, Any], config_file: str) -> List[HIPAAValidationIssue]:
        """Validate environment-specific requirements"""
        issues = []
        ...
        if "healthcare_system" in config_data:
            environment = config_data["healthcare_system"].get("metadata", {}).get("deployment_environment")
    ...
    def _validate_production_requirements(self, config_data: Dict[str, Any], config_file: str) -> List[HIPAAValidationIssue]:
    ...
    def _find_nested_section(self, config_data: Dict[str, Any], section_name: str) -> Optional[Dict[str, Any]]:
        ...
        if section_name in healthcare_system:
            return healthcare_system[section_name]
        ...
        if section_name in medical_compliance:
            return medical_compliance[section_name]
        
        # Look in security section
        security = healthcare_system.get("security", {})
        if section_name in security:
            return security[section_name]
        
        return None
    
    def run_validation(self, search_paths: List[str] = None) -> Tuple[int, int, int]:
        ...
        if not config_files:
            self.issues.append(HIPAAValidationIssue(
    ...
    def _generate_text_report(self) -> str:
        ...
        report_lines = [
            "🏥 HIPAA Configuration Validation Report",
            "=" * 45,
            ""
        ]
