...
class HealthcareComplianceChecker:
    ...
    def _get_synthetic_markers(self) -> List[str]:
        ...
        return [
            ...
            "synthetic", "_synthetic", "SYN-", "TEST-", "DEMO-",
            # Healthcare synthetic IDs
            "PAT001", "PAT002", "PAT003", "PAT999", "PROV001", "PROV002", "ENC001",
            # Fake contact information
            "000-000-0000", "555-0000", "XXX-XX-XXXX", "123-45-6789",
            "example.com", "test.com", "synthetic.test", "fake.email",
            # Test/demo names and organizations
            "John Doe", "Jane Smith", "Meghan Anderson", "Test Hospital",
            "UnitedHealth Test", "Demo Clinic", "Sample Healthcare",
            # Development markers
            "fake", "test", "demo", "mock", "stub", "placeholder",
            # Synthetic healthcare data indicators
            "generated_patient", "synthetic_encounter", "test_diagnosis"
        ]
    ...
    def _get_skip_directories(self) -> Set[str]:
        ...
        return {
            ...
            "data/synthetic", "test", "tests", "mock", "coverage",
            ".venv", "venv", "env", "reference/ai-patterns"
        }
    
    def is_synthetic_data(self, content: str, filename: str) -> bool:
        """Enhanced synthetic data detection"""
        # Check file path
        if any(skip_dir in filename for skip_dir in ["data/synthetic", "test", "mock", "_test", ".test."]):
            return True
    ...
    def run_compliance_check(self, target_paths: List[str] = None) -> Tuple[int, int, int]:
        ...
        for target_path in target_paths:
            if os.path.isfile(target_path):
                ...
                for root, dirs, files in os.walk(target_path):
                    # Skip certain directories
                    dirs[:] = [d for d in dirs if not any(skip in d for skip in self.skip_directories)]
...
Examples:
  ...
  %(prog)s src/ agents/             # Check specific directories
    ...
    parser.add_argument(
        "--no-phi-check",
        action="store_true",
        help="Disable PHI pattern checking"
    )
