...
class DeduplicationMonitor:
        ...
        try:
            ...
            for table in self.thresholds["min_expected_records"]:
                try:
                    ...
                    logging.exception(f"Failed to check {table}: {e}")
    ...
    async def _check_duplicates_by_table(self, conn: asyncpg.Connection, table: str) -> dict[str, Any]:
        ...
        try:
            ...
            unique_field = [k for k in result if k.startswith("unique_")][0]
    ...
    def _assess_source_status(self, record_count: int, deduplication_rate: float, table: str) -> str:
        ...
        if record_count < min_expected * 0.1 or deduplication_rate > self.thresholds["critical_duplication_rate"]:  # Less than 10% of expected
            return "critical"
        if deduplication_rate > self.thresholds["warning_duplication_rate"] or record_count < min_expected * 0.5:
            return "warning"
        return "healthy"
