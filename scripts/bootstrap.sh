...
# For development, use workspace-relative config directory
if [[ "${SCRIPT_DIR}" == "/home/intelluxe/scripts" ]] || [[ "${CI:-}" == "true" ]]; then
    : "${CFG_ROOT:=${SCRIPT_DIR%/scripts}/stack}"
else
    : "${CFG_ROOT:=/opt/intelluxe/stack}"
fi

# Set dependent configuration paths after CFG_ROOT is determined
...
fix_development_permissions() {
    log "🔧 Fixing development environment permissions..."
    ...
    if [[ "$is_development" != "true" ]]; then
        log "Not in development environment, skipping permission fixes"
        return 0
    fi

    # Fix script permissions to 775 (rwxrwxr-x) for collaborative development
    log "Setting script permissions to 775 for collaborative development..."
    local script_count=0
    local fixed_count=0

    # Work with the actual development directory structure
    local scripts_dir="${SCRIPT_DIR}"             # This should be /home/intelluxe/scripts
    local workspace_root="${SCRIPT_DIR%/scripts}" # This should be /home/intelluxe
    ...
    # Fix ownership to justin:intelluxe (1000:1001) for development files
    log "Setting ownership to justin:intelluxe (1000:1001) for development files..."
...
main() {
    ...
    # Fix development environment permissions automatically
    fix_development_permissions
