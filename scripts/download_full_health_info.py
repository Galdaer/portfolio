    ...
    async def download_complete_archive(self) -> dict[str, Any]:
        ...
        try:
            async with self.base_downloader:
                ...
                self.logger.info("✅ Complete health information download finished!")
    ...
    async def save_complete_datasets(self, validated_data: dict[str, list[dict]]) -> list[str]:
        ...
        for dataset_name, data in validated_data.items():
            ...
            if dataset_name == "health_topics":
                categories = list({item.get("category", "") for item in data if item.get("category")})
                dataset_info["metadata"]["categories"] = categories
                dataset_info["metadata"]["sources"] = list({item.get("source", "") for item in data})
            elif dataset_name == "exercises":
                body_parts = list({item.get("body_part", "") for item in data if item.get("body_part")})
                equipment_types = list({item.get("equipment", "") for item in data if item.get("equipment")})
                ...
                food_categories = list({item.get("food_category", "") for item in data if item.get("food_category")})
...
def main():
    ...
    if args.exercises_only and not rapidapi_key:
        ...
        return
    if args.nutrition_only and not usda_api_key:
    ...
    # Show next steps
    print("\n📋 Next Steps:")
    print("   1. Parse downloaded files: python scripts/parse_downloaded_archives.py health-info")
    print("   2. Or use medical-mirrors API: POST /update/health-info")
    ...
    if not usda_api_key:
        print("💡 Note: Set USDA_API_KEY to download USDA nutrition data")
